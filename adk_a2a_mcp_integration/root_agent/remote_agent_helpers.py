...
async def list_remote_agents() -> list[dict]:
    ...
    if remote_agent_cards_cache:
        print("Remote agents card cache exists, using cache")
        return remote_agent_cards_cache
    print("Agent card cache empty, fetching...")
    try:
...
async def call_remote_agent(query: str, agent_name: str) -> str:
    ...
    async with httpx.AsyncClient(timeout=120) as httpx_client:
        ...
        try:
            ...
            print(f"Response received from remote agent: {response_dict}")
        except Exception as e:
            print("Failed to send message to remote agent.")
